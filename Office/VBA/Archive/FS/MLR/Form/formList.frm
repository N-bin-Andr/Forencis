VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} formList 
   Caption         =   "Форма для расчета нагрузки за месяц"
   ClientHeight    =   12810
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10215
   OleObjectBlob   =   "formList.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "formList"
...
End Sub
'
Private Sub caseNum_Exit(ByVal Cancel As MSForms.ReturnBoolean)
'поле "номер экспертизы"
    With caseNum
        If .Text = "" Then
            .BackColor = &HFF&
        Else: .BackColor = &HC0FFC0
        End If
    End With
End Sub
'
...
'1) определение текущей даты:
    ...
    Dim str2 As String
        str2 = VBA.Format(dt, "mm/yyyy") 'текущий месяц/год
'2)печать в файл:
 Dim newFile As String, newNum As Integer
    ...
    Open PATH & newFile For Append As #fileDescr
        ...
        Print #fileDescr, "Заключение №"; caseNum.Text
