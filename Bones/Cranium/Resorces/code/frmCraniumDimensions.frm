...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmCraniumDimensions 
   Caption         =   "Краниометрический метод"
   ClientHeight    =   9165
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6630
...
Const IMGdir As String = "D:\Development\VBA\МойРабочийАрхив\CraniumDimension\VB\"
'
...
Private Static Function getCrForm() As Currency
    If dimension(1) > 0 And dimension(0) > 0 Then
        Dim tmp As Currency
         tmp = dimension(1) * 100 / dimension(0)
    Else: tmp = 0
    End If
    getCrForm = Format(tmp, "##0.0")
...
Private Static Function getCrHeight() As Currency
    If dimension(2) > 0 And dimension(0) > 0 Then
        Dim tmp As Currency
        tmp = dimension(2) * 100 / dimension(0)
    Else: tmp = 0
    End If
    getCrHeight = Format(tmp, "##0.0")
